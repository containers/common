...
func RuntimeConfigFilter(ctx context.Context, options RuntimeConfigFilterOptions) (hookErr, err error) {
	...
	for i, hook := range options.Hooks {
		...
		hookErr, err = Run(ctx, RunOptions{Hook: &hook, Dir: options.Dir, State: data, Stdout: &stdout, PostKillTimeout: options.PostKillTimeout})
		if err != nil {
			return hookErr, err
		}
