...
func TestRun(t *testing.T) {
	...
	hookErr, err := Run(ctx, RunOptions{Hook: hook, State: []byte("{}"), Stdout: &stdout, Stderr: &stderr, PostKillTimeout: DefaultPostKillTimeout})
	if err != nil {
		t.Fatal(err)
	}
	if hookErr != nil {
...
func TestRunIgnoreOutput(t *testing.T) {
	...
	hookErr, err := Run(ctx, RunOptions{Hook: hook, State: []byte("{}"), PostKillTimeout: DefaultPostKillTimeout})
	if err != nil {
		t.Fatal(err)
	}
	if hookErr != nil {
...
func TestRunCwd(t *testing.T) {
	...
	var stderr, stdout bytes.Buffer
	hookErr, err := Run(ctx, RunOptions{Hook: hook, Dir: cwd, State: []byte("{}"), Stdout: &stdout, Stderr: &stderr, PostKillTimeout: DefaultPostKillTimeout})
	if err != nil {
		t.Fatal(err)
	}
	if hookErr != nil {
...
func TestRunKillTimeout(t *testing.T) {
	...
	hookErr, err := Run(ctx, RunOptions{Hook: hook, State: []byte("{}"), PostKillTimeout: time.Duration(0)})
